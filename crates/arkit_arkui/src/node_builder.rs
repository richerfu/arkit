...
impl NodeBuilder {
    ...
    pub fn child(mut self, child: OwnedNativeNode) -> ArkUIResult<Self> {
        // ArkUI takes ownership of the mounted wrapper `Rc`; the parent owns
        // disposal of the native subtree from this point onward.
        let child = std::rc::Rc::new(std::cell::RefCell::new(child.into_raw()));
        let result = self
            .node
            .as_mut()
            .expect("NodeBuilder owns a node until build")
